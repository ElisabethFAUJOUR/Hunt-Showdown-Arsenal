const app = {
  ...
  toggleNavMenu() {
    ...
    const linksElem = document.querySelectorAll(".nav-link");
  ...
  sortCardsByData(itemType, cardsArray, sortDirection, sortData) {
    ...
    const itemsWithSortData = cardsArray.filter((card) =>
      card.querySelector(`[data-value="${sortData}"]`)
    );

    const sortedItems = itemsWithSortData.sort((a, b) => {
      const valueA = parseFloat(
        a.querySelector(`[data-value="${sortData}"]`).textContent
      );
      const valueB = parseFloat(
        b.querySelector(`[data-value="${sortData}"]`).textContent
      );
